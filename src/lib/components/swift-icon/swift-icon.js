...
const SwiftIcon = (props) => {
  const { i, className, color } = props;
  if (!i) return <></>;

  if (Object.keys(icons).includes(i))
    return (
      <SwiftIconStyled className={className} color={color}>
        {icons[i]}
      </SwiftIconStyled>
    );
