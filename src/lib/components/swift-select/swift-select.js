...
const SwiftSelect = (props) => {
  ...
  return (
    ...
    >
      ...
      {props.sublabel ? <SwiftLabel theme={theme} style={{ marginTop: '8px', opacity: 0.4 }}  className="swift_input_text_sublabel">{props.sublabel}</SwiftLabel> : null}
      {props.error && props.helperText ? <SwiftLabel theme={theme} error  className="swift_input_text_label_error">{props.helperText}</SwiftLabel> : null}
    </SwiftSelectStyled>
  );
};
